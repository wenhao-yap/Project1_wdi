...
function update(){
    ...
    // console.log(avatar.x_spd);
    if(storeKey[leftKey]){
        if(avatar.x_spd > -2){
            avatar.idleToLeft = true;
            avatar.x_spd--;
        }
        if(avatar.x_spd > -5 && avatar.x_spd < -2){
            avatar.running = true;
            avatar.x_spd--;
        }       
    }
    if(storeKey[rightKey]){
        avatar.idleToLeft = false;
        if(avatar.x_spd < 2){
            avatar.running = false;
            avatar.x_spd++;
        }
        else if(avatar.x_spd < 5 && avatar.x_spd >2){
            avatar.running = true;
            avatar.x_spd++
        }
    }
